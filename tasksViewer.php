	...
	public function populateControl($tasks) {
		...
		foreach ($tasks as $task=>$value)
		{
			...
			<li class="page-item task-item <?php if ($c <= 3) echo "curr" ?>">
				...
				<div class="list-group-item"> <?php if($value["isDone"]) echo "Done!"; ?> </div>
				<?php } else { ?>
				...
				<div class="list-group-item list-group-item-action"><input type="checkbox" class="isDone-ch" onchange="updateTask(this)" <?php if($value["isDone"]) { echo "checked"; ?> /><?php echo " Done!"; } ?></div>
				<?php } ?>
			</li>
			<?php
		}
